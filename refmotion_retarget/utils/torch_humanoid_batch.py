...
class Humanoid_Batch:
    ...
    def __init__(self, cfg, device = torch.device("cpu")):
        self.cfg = cfg
        self.mjcf_file = os.path.join(cfg.asset.assetRoot, cfg.asset.assetFileName)
        logger.info(f"Humanoid_Batch using mjcf file: {self.mjcf_file}")
    ...
    def _compute_velocity(p, time_delta, guassian_filter=True):
        velocity = np.gradient(p.detach().numpy(), axis=-3) / time_delta
        if guassian_filter:
            velocity = torch.from_numpy(filters.gaussian_filter1d(velocity, 1, axis=-3, mode="nearest")).to(p)
        else:
    ...
    def _compute_angular_velocity(r, time_delta: float, guassian_filter=True):
        ...
        if guassian_filter:
            angular_velocity = torch.from_numpy(filters.gaussian_filter1d(angular_velocity.detach().numpy(), 1, axis=-3, mode="nearest"),)
    ...
    def load_mesh(self):
        ...
        for mesh_file_node in tqdm(all_mesh, desc=f"loading mesh file node ..."):
        ...
        mesh_to_body = {}
        for geom_node in tqdm(geoms,desc=f"loading geom node pos and quat ..."):
    ...
    def mesh_fk(self, pose = None, trans = None):
        ...
        for geom in geoms:
            ...
            body_trans = g_trans[body_idx].numpy().copy() # link or body local frame
